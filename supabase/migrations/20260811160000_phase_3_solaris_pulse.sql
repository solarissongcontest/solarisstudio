-- ============================================================
-- SOLARIS STUDIO — PHASE 3: SOLARIS PULSE
--
-- Run this AFTER scripts/lovable-phase-2-3-setup.sql has already
-- completed successfully. It extends the existing Pulse foundation;
-- it does not recreate or wipe Prediction Arena data.
--
-- Adds:
--   * automatic publication events for editions/shows/prediction rounds
--   * event deduplication
--   * bulk "mark all read"
--   * privacy-safe prediction-consensus snapshots and movement RPC
-- ============================================================

-- ------------------------------------------------------------
-- 1. CONTENT EVENT EXTENSIONS + DEDUPLICATION
-- ------------------------------------------------------------

alter table public.content_events
  add column if not exists dedupe_key text;

create unique index if not exists content_events_dedupe_key_idx
on public.content_events (dedupe_key);
...
create or replace function public.mark_content_events_read(
  _event_ids uuid[]
)
returns integer
language plpgsql
...
begin
  ...
  insert into public.fan_event_reads (
    profile_id,
    event_id,
    read_at
  )
...
revoke all
on function public.mark_content_events_read(uuid[])
from public, anon;
...
-- ------------------------------------------------------------
-- 3. AUTOMATIC EDITION PUBLICATION EVENTS
-- ------------------------------------------------------------
...
drop trigger if exists trg_emit_edition_pulse_event
on public.editions;
...
on public.editions
for each row
execute function public.emit_edition_pulse_event();

-- ------------------------------------------------------------
-- 4. AUTOMATIC SHOW PUBLICATION EVENTS
-- ------------------------------------------------------------
...
begin
  ...
  if coalesce((new_config ->> 'running_order')::boolean, false)
    and (
      not old_show_was_public
      or not coalesce((old_config ->> 'running_order')::boolean, false)
    )
  then
    insert into public.content_events (
      event_type,
    ...
    values (
      ...
      now(),
      'show:' || new.id::text || ':running-order'
    )
    on conflict (dedupe_key)
    do update set
      title = excluded.title,
  ...
  if coalesce((new_config ->> 'results')::boolean, false)
    and (
      not old_show_was_public
      or not coalesce((old_config ->> 'results')::boolean, false)
    )
  then
    insert into public.content_events (
      event_type,
...
drop trigger if exists trg_emit_show_pulse_events
on public.shows;
...
on public.shows
for each row
execute function public.emit_show_pulse_events();

-- ------------------------------------------------------------
-- 5. AUTOMATIC PREDICTION OPEN/CLOSE EVENTS
-- ------------------------------------------------------------
...
drop trigger if exists trg_emit_prediction_round_pulse_events
...
on public.prediction_rounds
for each row
execute function public.emit_prediction_round_pulse_events();

-- ------------------------------------------------------------
-- 6. PRIVACY-SAFE PREDICTION CONSENSUS SNAPSHOTS
-- ------------------------------------------------------------

create table if not exists public.prediction_consensus_snapshots (
  ...
  unique (round_id, snapshot_hash)
);

create index if not exists prediction_consensus_snapshots_round_time_idx
on public.prediction_consensus_snapshots (round_id, captured_at desc);
...
create or replace function public.capture_prediction_consensus_snapshot(
  _round_id uuid
)
returns void
language plpgsql
...
revoke all
on function public.capture_prediction_consensus_snapshot(uuid)
...
drop trigger if exists trg_capture_prediction_consensus_snapshot
on public.prediction_entry_versions;

create trigger trg_capture_prediction_consensus_snapshot
after insert
on public.prediction_entry_versions
for each row
execute function public.capture_prediction_snapshot_after_submission();
...
begin
  for round_row in
    select id from public.prediction_rounds
    where status not in ('draft', 'cancelled')
  loop
    perform public.capture_prediction_consensus_snapshot(round_row.id);
  end loop;
end;
$$;
...
create or replace function public.prediction_consensus_movement(
  _round_id uuid
)
returns jsonb
language plpgsql
...
begin
  ...
  then
    raise exception 'Submit a prediction before viewing consensus movement';
  end if;
  ...
  from (
    select *
    from public.prediction_consensus_snapshots
...
revoke all
on function public.prediction_consensus_movement(uuid)
from public, anon;
...
comment on function public.prediction_consensus_movement(uuid) is
  'Returns the two newest privacy-safe aggregate prediction snapshots for meaningful movement UI.';

comment on column public.content_events.dedupe_key is
  'Stable key used by automatic Pulse generators so repeated publication edits do not create duplicate inbox events.';
