...
as $$
declare
  v_old_outcome_public boolean := false;
  v_new_outcome_public boolean := false;
  v_ops public.studio2_result_operations%rowtype;
begin
  if tg_op = 'UPDATE' then
    v_old_outcome_public :=
      coalesce(old.published, false)
      and (
        coalesce((old.publication_config ->> 'qualifiers')::boolean, false)
        or coalesce((old.publication_config ->> 'results')::boolean, false)
  ...
  if v_new_outcome_public and not v_old_outcome_public then
    select *
      into v_ops
