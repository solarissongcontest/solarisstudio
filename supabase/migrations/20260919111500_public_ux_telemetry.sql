...
commit;
    ...
    'elapsed_ms', case
      when coalesce(v_metadata ->> 'elapsed_ms', '') ~ '^[0-9]{1,8}
...
commit;
        ...
        then least((v_metadata ->> 'elapsed_ms')::int, 86400000)
      else null
    end,
    'start_route', left(nullif(v_metadata ->> 'start_route', ''), 120),
    'search_used', case
      when v_metadata ->> 'search_used' in ('yes', 'no')
        then v_metadata ->> 'search_used'
      else null
    end
  ));
