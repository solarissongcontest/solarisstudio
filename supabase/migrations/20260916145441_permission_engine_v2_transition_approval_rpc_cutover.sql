...
-- Permission Engine v2 cutover batch 11.
--
-- Transition approvals already admit edition-management specialists alongside
-- legacy Organizers. Preserve those non-strict specialist paths and the later
-- lock-order hardening while removing direct legacy role checks. Archival keeps
-- its distinct edition.archive capability through the existing dynamic mapping.
...
begin
  ...
  -- Preserve the lock-order hardening: resolve scope first without a row lock,
  -- authorize, lock the edition runtime, then lock the approval request.
  select * into v_request
  from public.studio2_transition_approval_requests
  ...
  if not public.studio2_access_allowed('edition.manage', v_request.edition_id, false) then
    raise exception 'Missing Solaris capability: edition.manage' using errcode = '42501';
  end if;
...
begin
  ...
  if not public.studio2_access_allowed(v_required_capability, p_edition_id, false) then
...
revoke all on function public.studio2_request_transition_approval(uuid, text, text) from public, anon;
grant execute on function public.studio2_request_transition_approval(uuid, text, text) to authenticated, service_role;

revoke all on function public.studio2_approve_transition(uuid) from public, anon;
grant execute on function public.studio2_approve_transition(uuid) to authenticated, service_role;

revoke all on function public.studio2_list_transition_approvals(uuid) from public, anon;
grant execute on function public.studio2_list_transition_approvals(uuid) to authenticated, service_role;

revoke all on function public.studio2_transition_edition_v2(uuid, text, text, uuid) from public, anon;
grant execute on function public.studio2_transition_edition_v2(uuid, text, text, uuid) to authenticated, service_role;
