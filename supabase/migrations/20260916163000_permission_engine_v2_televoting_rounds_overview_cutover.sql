...
-- Permission Engine v2 cutover batch 15.
--
-- The merged televoting rounds page is a read-only voting-operations surface.
-- Replace its direct Organizer requirement with the same edition-scoped,
-- non-strict voting.read capability used by the Permission Engine route map.
...
begin
  if auth.uid() is null
     or not public.studio2_access_allowed('voting.read', p_solaris_edition_id, false) then
    raise exception 'Voting operations read capability required' using errcode = '42501';
  end if;
...
revoke execute on function televoting.admin_rounds_page_overview(uuid)
  from public, anon, service_role;
grant execute on function televoting.admin_rounds_page_overview(uuid)
  to authenticated;
