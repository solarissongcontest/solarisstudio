...
-- Permission Engine v2 cutover batch 10.
--
-- Incident Command already admits incident.manage specialists alongside legacy
-- Organizers, so these paths use non-strict cutover semantics before the
-- authoritative switch. Preserve validation, locking, lifecycle and event
-- behavior; replace only the legacy authorization branch.
...
begin
  ...
  if not public.studio2_access_allowed('incident.manage', p_edition_id, false) then
...
begin
  ...
  if not public.studio2_access_allowed('incident.manage', v_incident.edition_id, false) then
    raise exception 'Missing Solaris capability: incident.manage' using errcode = '42501';
  end if;
...
revoke all on function public.studio2_create_incident(uuid, text, text) from public, anon;
grant execute on function public.studio2_create_incident(uuid, text, text) to authenticated, service_role;

revoke all on function public.studio2_create_incident_v2(uuid, text, text, text, text, text[]) from public, anon;
grant execute on function public.studio2_create_incident_v2(uuid, text, text, text, text, text[]) to authenticated, service_role;

revoke all on function public.studio2_update_incident(uuid, text, text, text, text, text[], uuid, boolean, text, text) from public, anon;
...
revoke all on function public.studio2_acknowledge_incident(uuid) from public, anon;
grant execute on function public.studio2_acknowledge_incident(uuid) to authenticated, service_role;

revoke all on function public.studio2_declare_incident_crisis(uuid) from public, anon;
grant execute on function public.studio2_declare_incident_crisis(uuid) to authenticated, service_role;

revoke all on function public.studio2_add_incident_timeline_event(uuid, text) from public, anon;
grant execute on function public.studio2_add_incident_timeline_event(uuid, text) to authenticated, service_role;

revoke all on function public.studio2_transition_incident(uuid, text) from public, anon;
