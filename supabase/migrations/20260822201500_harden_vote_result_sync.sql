...
begin
  if old.voting_config is distinct from new.voting_config then
    perform public.sync_show_results_from_votes(new.id);
  end if;
  return new;
end;
$$;

create or replace function public.sync_results_after_participant_change()
returns trigger
language plpgsql
security definer
set search_path = public, pg_temp
...
after insert or delete or update of show_id, country_id, contest_entity_id, participation_status, running_order
...
declare
  row_show record;
begin
  for row_show in select id from public.shows loop
    perform public.sync_show_results_from_votes(row_show.id);
  end loop;
end $$;