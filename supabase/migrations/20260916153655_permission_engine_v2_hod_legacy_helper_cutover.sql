...
-- Permission Engine v2 cutover batch 14.
--
-- The legacy HOD roster helper is service-role-only as a direct RPC but is
-- invoked by the authenticated studio2_hod_context wrapper. Its legacy
-- Organizer bit controls both authorization and whether notice acknowledgements
-- are evaluated for the target country rather than only the caller. Replace
-- that bit with the same non-strict confirmation.manage inspector semantics in
-- both places so pre-cutover behavior is preserved and authoritative cutover
-- removes the legacy Organizer bypass cleanly.

create or replace function public.studio2_hod_context_legacy_roster(
  p_edition_id uuid,
  p_country_id uuid
)
returns jsonb
...
begin
  ...
  v_can_manage_country := coalesce(
    public.studio2_access_allowed('confirmation.manage', p_edition_id, false),
    false
  );
  ...
  -- belong here. Staff/country-manager inspection evaluates the target country
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'acknowledged', case
          when v_is_service or v_can_manage_country then exists (
            select 1
