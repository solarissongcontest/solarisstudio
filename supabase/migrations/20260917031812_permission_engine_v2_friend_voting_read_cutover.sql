...
-- Recreate the canonical Friend Voting historical-country analytics RPC so clean
-- migration replay no longer depends on a production-only historical fastpath.
-- The analytics body is preserved; only the legacy Organizer gate moves to
-- edition/global non-strict voting.read compatibility semantics.
...
CREATE OR REPLACE FUNCTION public.friend_voting_historical_country_payload(p_channel text DEFAULT 'combined'::text, p_edition_id uuid DEFAULT NULL::uuid, p_limit integer DEFAULT 250)
 ...
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'televoting', 'pg_temp'
AS $function$
declare
...
begin
  if coalesce(auth.role(), '') <> 'service_role'
    ...
    raise exception 'Voting read capability required' using errcode = '42501';
  end if;
