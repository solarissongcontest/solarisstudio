...
-- Restore the known SSC4 historical order after a previous vote-sync fallback
-- reordered fully tied entries by UUID. Names are used deliberately so the
-- migration does not depend on generated database ids.
with target_show as (
  select s.id
  from public.shows s
  join public.editions e on e.id = s.edition_id
  where e.edition_number = 4
    and lower(btrim(s.name)) = 'grand final'
  order by s.created_at
  limit 1
), historical(country_name, final_rank) as (
  values
    ('Intago', 1),
    ('Elaria', 2),
    ('Skandia', 3),
...
update public.results r
set final_rank = h.final_rank,
    updated_at = now()
from target_show ts, historical h, public.countries c
where r.show_id = ts.id
  and r.country_id = c.id
  and lower(btrim(c.name)) = lower(h.country_name);
