-- Country-account HOD history. Delegations can mark which past editions were
-- controlled by the current HOD, another HOD, or an unknown HOD. New canonical
-- participations carry the active HOD forward unless they explicitly stop it.
...
create table if not exists public.country_hod_edition_claims (
  ...
  status text not null check (status in ('mine','other','unknown')),
