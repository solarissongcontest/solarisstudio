alter table public.national_finals
  ...
  add column if not exists source text not null default 'confirmation';

update public.national_finals nf
...
begin
  if not exists (
    ...
    where conrelid = 'public.national_finals'::regclass
      and conname = 'national_finals_source_check'
  ) then
    alter table public.national_finals
      add constraint national_finals_source_check check (source in ('confirmation','manual'));
  end if;
  if not exists (
    select 1 from pg_constraint
    where conrelid = 'public.national_finals'::regclass
      and conname = 'national_finals_owner_source_check'
  ) then
    alter table public.national_finals
      add constraint national_finals_owner_source_check check (submission_id is not null or country_id is not null);
  end if;
end $$;

create index if not exists national_finals_country_edition_idx
  on public.national_finals(country_id, edition_id);
...
as $$
  select exists(
    ...
    auth.uid() is not null and public.has_role(auth.uid(), 'organizer'::public.app_role)
  );
$$;
...
begin
  ...
  for v_entry in select value from jsonb_array_elements(coalesce(_entries,'[]'::jsonb)) loop
    ...
    insert into public.national_final_entries(
      ...
      review_status, review_reason, reviewed_at, removed
    ) values (
      v_nf_id,
...
grant execute on function public.delete_country_historical_national_final(uuid,uuid) to authenticated;
grant execute on function public.can_manage_country_national_finals(uuid) to authenticated;
