alter table public.friend_voting_settings
  add column if not exists advanced_model_config jsonb not null default jsonb_build_object(
...
begin
  select * into v_before from public.friend_voting_settings where id = 'default' for update;
  if not found then
    insert into public.friend_voting_settings (id) values ('default') returning * into v_before;
  end if;
  ...
  set
    ...
    advanced_model_config = coalesce(p_values->'advanced_model_config', advanced_model_config)
  where id = 'default'
  returning * into v_after;

  insert into public.admin_audit_log (actor_id, action, table_name, record_id, before_data, after_data)
  values (p_actor_id, 'update_friend_voting_settings', 'friend_voting_settings', 'default', to_jsonb(v_before), to_jsonb(v_after));
...
comment on column public.friend_voting_settings.advanced_model_config is
  'Organizer-controlled JSON configuration for friend-voting model v2 statistical weights and small-sample safeguards.';
