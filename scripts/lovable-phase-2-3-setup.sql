-- SOLARIS STUDIO — LOVABLE SQL EDITOR SETUP
-- Phase 2 complete + Phase 3 foundation
-- Paste this entire file into Lovable's connected Supabase SQL Editor and run once.
-- The transaction prevents a partially applied setup.

begin;

-- Make the script safe to retry after a previous partial run.
do $$
begin
  if to_regclass('public.fan_profiles') is not null then
    execute 'drop policy if exists "fans read own profile" on public.fan_profiles';
    execute 'drop policy if exists "fans create own profile" on public.fan_profiles';
    execute 'drop policy if exists "fans update own profile" on public.fan_profiles';
    execute 'drop policy if exists "fans delete own profile" on public.fan_profiles';
  end if;
  if to_regclass('public.prediction_rounds') is not null then
    execute 'drop policy if exists "public reads published prediction rounds" on public.prediction_rounds';
    execute 'drop policy if exists "organizers manage prediction rounds" on public.prediction_rounds';
  end if;
  if to_regclass('public.prediction_entries') is not null then
    execute 'drop policy if exists "fans read own prediction entries" on public.prediction_entries';
  end if;
  if to_regclass('public.prediction_items') is not null then
    execute 'drop policy if exists "fans read own prediction items" on public.prediction_items';
  end if;
  if to_regclass('public.prediction_entry_versions') is not null then
    execute 'drop policy if exists "fans read own prediction versions" on public.prediction_entry_versions';
  end if;
  if to_regclass('public.prediction_scores') is not null then
    execute 'drop policy if exists "fans read own prediction scores" on public.prediction_scores';
  end if;
  if to_regclass('public.fan_follows') is not null then
    execute 'drop policy if exists "fans read own follows" on public.fan_follows';
  end if;
  if to_regclass('public.content_events') is not null then
    execute 'drop policy if exists "public reads published content events" on public.content_events';
    execute 'drop policy if exists "organizers manage content events" on public.content_events';
  end if;
  if to_regclass('public.fan_event_reads') is not null then
    execute 'drop policy if exists "fans read own event state" on public.fan_event_reads';
  end if;
  if to_regclass('public.notification_preferences') is not null then
    execute 'drop policy if exists "fans read own notification preferences" on public.notification_preferences';
    execute 'drop policy if exists "fans create own notification preferences" on public.notification_preferences';
    execute 'drop policy if exists "fans update own notification preferences" on public.notification_preferences';
    execute 'drop policy if exists "fans delete own notification preferences" on public.notification_preferences';
  end if;
end;
$$;
