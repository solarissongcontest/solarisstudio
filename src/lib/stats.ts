...
type CombinedEditionPlacement = {
  ...
  rank: number | null;
  row: ResultRow;
  source: "final" | "semi" | "other";
};

/**
 * Builds one overall placement table per edition.
 *
 * Grand Finalists keep their official final rank. Countries that have a
 * semi-final result but no Grand Final result are treated as NQs and are
 * placed immediately after the finalists, ordered by semi-final points.
 *
 * Example: 26 finalists + 10 NQs => one overall table ranked 1-36.
 */
function buildCombinedEditionPlacements(
  results: ResultRow[],
  shows: Show[],
): Map<string, Map<string, CombinedEditionPlacement>> {
  ...
  for (const [editionId, rows] of byEdition.entries()) {
    ...
    if (finalRows.length) {
      ...
      for (const row of finalRows) {
        finalistIds.add(row.country_id);
      ...
      const finalRankMax = Math.max(
        finalRows.length,
        ...finalRows.map((row) => row.final_rank ?? 0),
      );

      // A country should have only one semi-final result per edition, but if
      // old data contains more than one, keep its strongest semi performance.
      ...
      for (const row of rows) {
        ...
        if (finalistIds.has(row.country_id)) {
          continue;
        }

        const current = nqSemiByCountry.get(row.country_id);

        if (
          !current ||
          row.total_points > current.total_points ||
          (row.total_points === current.total_points &&
            row.televote_points > current.televote_points) ||
          (row.total_points === current.total_points &&
            row.televote_points === current.televote_points &&
            row.jury_points > current.jury_points)
        ) {
          nqSemiByCountry.set(row.country_id, row);
        }
      }

      const nqRows = [...nqSemiByCountry.values()].sort((a, b) => {
        if (b.total_points !== a.total_points) {
          return b.total_points - a.total_points;
        }

        if (b.televote_points !== a.televote_points) {
          return b.televote_points - a.televote_points;
        }

        if (b.jury_points !== a.jury_points) {
          return b.jury_points - a.jury_points;
        }

        return a.country_id.localeCompare(b.country_id);
      });

      nqRows.forEach((row, index) => {
        placements.set(row.country_id, {
          editionId,
          countryId: row.country_id,
          rank: finalRankMax + index + 1,
          row,
          source: "semi",
        });
      });
    } else {
      // Before an edition has a Grand Final result, keep at most one row per
      // country so an unfinished edition can never create duplicate timeline
      // points. This is provisional and is replaced automatically once the
      // Grand Final exists.
      const bestByCountry = new Map<string, ResultRow>();

      for (const row of rows) {
        const current = bestByCountry.get(row.country_id);

        if (
          !current ||
          (row.final_rank != null && current.final_rank == null) ||
          (row.final_rank != null &&
...
export function computeCountryStats(
  ...
  const timeline: CountryTimelinePoint[] = opts.editions
    .map((edition) => {
      const placement = combinedPlacements.get(edition.id)?.get(countryId);
  ...
  // each edition, so an edition can never appear twice and NQs are included
  // directly after the finalists.
  const placementTimeline = timeline.filter((point) => point.rank != null);
  ...
  const rolling5 = placementTimeline.map((_, index) => {
    ...
    return {
      ...
      editionNumber: placementTimeline[index].editionNumber,
  ...
  for (let index = 1; index < placementTimeline.length; index += 1) {
    const previous = placementTimeline[index - 1];
    const current = placementTimeline[index];
