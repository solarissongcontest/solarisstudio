import {
  SSC_RULES as SOURCE_RULES,
  type RuleTone,
  type SscRule,
  type SscRuleChapter,
} from "@/lib/ssc-rules-online-base";
...
export function getRuleById(id: string) {
  ...
  return rulesById.get(normalized) ?? rulesById.get(legacyToCurrent.get(normalized) ?? "") ?? null;
}

export function getCurrentRuleId(id: string) {
  return rulesById.has(id) ? id : legacyToCurrent.get(id) ?? null;
}
...
export function searchSscRules(query: string) {
  ...
  return SSC_RULES.map((rule) => {
    const legacyIds = Object.entries(LEGACY_RULE_ALIASES).filter(([, current]) => current === rule.id).map(([legacy]) => legacy).join(" ");
    const haystack = normalize([rule.id, legacyIds, rule.title, rule.summary, rule.chapterTitle, rule.tags.join(" "), rule.body.join(" "), (rule.bullets ?? []).join(" "), (rule.allowed ?? []).join(" "), (rule.prohibited ?? []).join(" "), (rule.examples ?? []).map((example) => `${example.title} ${example.detail}`).join(" ")].join(" "));
    ...
    return { rule, score };
  }).filter(({ score }) => score > 0).sort((a, b) => b.score - a.score || a.rule.id.localeCompare(b.rule.id, undefined, { numeric: true })).map(({ rule }) => rule);
}
