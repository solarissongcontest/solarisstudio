...
const migration = readFileSync(
  resolve(process.cwd(), "supabase/migrations/20260910220400_rule_interpretations.sql"),
  "utf8",
);
...
const rulePage = readFileSync(
  resolve(process.cwd(), "src/routes/rules/$ruleId.tsx"),
  "utf8",
);
...
describe("Official Interpretations governance", () => {
  ...
  it("keeps an exact server-side registry of current v4 rule IDs", () => {
    ...
    expect(migration).toContain("select 1 from public.ssc_rule_registry where rule_id = v_normalized");
  });

  it("keeps direct tables closed and exposes only narrow RPCs", () => {
    expect(migration).toContain("alter table public.ssc_rule_interpretations enable row level security");
    expect(migration).toContain("revoke all on public.ssc_rule_interpretations from anon, authenticated");
  ...
  it("makes published interpretation wording immutable", () => {
    expect(migration).toContain("Published interpretations are immutable; supersede them with a new interpretation");
  ...
  it("requires explicit supersession instead of overwriting public history", () => {
    expect(migration).toContain("superseded_by uuid null references public.ssc_rule_interpretations");
    ...
    expect(migration).toContain("Replacement interpretation must address at least one of the same rules");
  ...
  it("publishes only effective interpretations to participants", () => {
    ...
    expect(migration).toContain("grant execute on function public.public_rule_interpretations(text) to anon, authenticated");
  ...
  it("survives organizer-account deletion without invalidating published history", () => {
    expect(migration).toContain("published_by uuid null references auth.users(id) on delete set null");
    expect(migration).not.toContain("status = 'published' and published_at is not null and published_by is not null");
    expect(migration).not.toContain("status = 'superseded' and published_at is not null and published_by is not null");
  });
