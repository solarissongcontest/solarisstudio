...
const allIdentityCopy = [
  participantCopy,
  organizerCase.toLowerCase(),
  privacyDoc.toLowerCase(),
].join("\n");
...
describe("Trust & Integrity identity privacy copy", () => {
  ...
  it("describes Fully Anonymous as an application case-data guarantee, not network invisibility", () => {
    ...
    expect(identityModel).toContain(
      "TSBC case reviewers do not receive a Solaris account identity from the case",
    );
    ...
    expect(privacyDoc).toContain(
      "Hosting, CDN, database or platform infrastructure may maintain logs",
    );
  ...
  it("describes Sealed Identity with the enforced two-person break-glass limits", () => {
    ...
    expect(organizerCase).toContain(
      "A different organizer must approve or reject it; self-approval is not available.",
    );
    expect(organizerCase).toContain("Reveal sealed identity once");
    expect(privacyDoc).toContain(
      "Only the organiser who originally requested disclosure may use that approval",
    );
  });
  ...
  it("describes Confidential Identity as explicit audited access rather than hidden forever", () => {
    ...
    expect(identityModel).toContain(
      "Access requires an explicit organizer action which is written to the case audit trail.",
    );
    expect(organizerCase).toContain(
      "Identity is hidden by default. Explicit access is audit-logged.",
    );
  ...
  it("keeps protected and anonymous recovery promises distinct in participant UI", () => {
    ...
    expect(protectedUi).toContain("For no account link at all");
    expect(protectedUi).toContain("use Fully Anonymous instead.");
    expect(anonymousUi).toContain("Keep a recovery key to");
    expect(anonymousUi).toContain("return and talk with TSBC.");
