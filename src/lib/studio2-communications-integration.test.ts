...
const migration = source(
  "supabase/migrations/20260911194000_studio2_official_communications_full.sql",
);
...
describe("Studio 2 Official Communications integration", () => {
  ...
  it("supports unread, read, acknowledgement and archive states on the HOD side", () => {
    expect(hodRoute).toMatch(/["']unread["']/);
    expect(hodRoute).toMatch(/["']read["']/);
    expect(hodRoute).toMatch(/["']acknowledgement_required["']/);
    expect(hodRoute).toMatch(/["']acknowledged["']/);
    expect(hodRoute).toMatch(/["']archived["']/);
