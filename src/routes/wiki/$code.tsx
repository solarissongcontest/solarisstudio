...
function CountryWikiPage() {
  ...
  const funFactsOverride =
    sections.find((section) => sectionSystemSlot(section) === "fun-facts") ?? null;
  ...
  const authoredOverview =
    profile?.summary?.trim() || country.description?.trim() || generatedOverview;
  ...
  return (
    <AppShell>
      <div className="mx-auto w-full max-w-none">
        ...
        <div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1fr)_330px] lg:items-start 2xl:grid-cols-[minmax(0,1fr)_360px] 2xl:gap-8">
          <article className="min-w-0 space-y-6">
            <WikiSection title="Overview">
              <p className="whitespace-pre-wrap">{authoredOverview}</p>
              {profile?.motto && (
                <blockquote className="mt-4 border-l-2 border-primary/50 pl-4 font-display italic text-foreground">
                  “{profile.motto}”
                </blockquote>
              )}
            </WikiSection>
                          ...
                          >
                            <div className="grid min-w-0 grid-cols-[1fr_auto] gap-3">
                              ...
                              <div className="flex items-center gap-2">
                                {label && (
                                  <span
                                    className={`rounded-full px-2 py-1 text-[9px] font-bold uppercase ${qualificationBadgeClass(qualification)}`}
                                  >
                                    {label}
                                  </span>
                                )}
                                {edition ? (
                                  <Link
                                    ...
                                    className="self-center text-[10px] font-semibold text-primary"
                                  >
                                    View →
                                  </Link>
                                ) : null}
              ...
              {character.tags.length > 0 && (
                <div className="mt-4 flex flex-wrap gap-2">
                  {character.tags.map((tag) => (
                    <span
                      key={tag}
                      className="rounded-full border border-border bg-surface px-3 py-1.5 text-[10px] font-semibold"
                    >
                      {tag}
                    </span>
                  ))}
          ...
          <aside className="min-w-0 lg:sticky lg:top-24">
            <div className="country-personality-card glass overflow-hidden">
              <div className="border-b border-border/60 p-4 text-center">
                ...
                {profile?.motto && (
                  <p className="mt-2 text-[11px] italic text-muted-foreground">“{profile.motto}”</p>
                )}
              </div>
              <div className="divide-y divide-border/50 px-4">
                {infoRows.map(([label, value]) =>
                  value ? (
                    <div
                      key={label}
                      className="grid grid-cols-[92px_minmax(0,1fr)] gap-3 py-3 text-xs"
                    >
              ...
              <div className="border-t border-border/60 p-4">
                <Link
                  to="/countries/$code"
                  params={{ code: country.short_code }}
                  className="flex min-h-10 items-center justify-center rounded-xl border border-border bg-surface px-3 text-xs font-semibold"
                >
                  ← Country overview
                </Link>
              </div>
...
function WikiSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="country-personality-card min-w-0 border-b border-border/60 p-4 sm:p-5">
      <h2 className="border-b border-border/60 pb-2 font-display text-xl font-semibold sm:text-2xl">
        {title}
      </h2>
...
function MiniStat({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="country-personality-inset rounded-xl bg-surface p-3">
