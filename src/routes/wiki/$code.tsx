    ...
    <AppShell>
      <div className="mx-auto max-w-[1160px]">
        <div className="mb-3 flex flex-wrap items-center gap-2 text-[11px] text-muted-foreground">
          ...
          <span>/</span>
          <Link to="/countries/$code" params={{ code: country.short_code }} className="hover:text-foreground">
            {country.name}
          </Link>
          <span>/</span>
        ...
        <section className="glass relative mb-5 overflow-hidden px-5 py-6 sm:px-7 sm:py-8">
          <BackgroundFlag
            image={country.flag_image}
            className="-right-20 -top-24 h-80 w-80"
            opacity={0.1}
          />
        ...
        <div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1fr)_310px] lg:items-start">
          <article className="min-w-0 space-y-6">
            ...
            {sections.map((section) => (
              <WikiSection key={section.id} title={section.heading}>
                <div className={section.image_url ? "grid gap-4 md:grid-cols-[minmax(0,1fr)_240px]" : ""}>
                  ...
                  {section.image_url && (
                    <figure className="min-w-0">
                      ...
                      {section.image_caption && (
                        <figcaption className="mt-2 text-[10px] leading-relaxed text-muted-foreground">
                          {section.image_caption}
                        </figcaption>
                      )}
                    </figure>
                  ...
                  {latestEntries.length > 0 && (
                    <div className="mt-5 overflow-hidden rounded-xl border border-border/70">
                      {latestEntries.map((entry) => {
                        ...
                        return (
                          <div key={entry.id} className="grid min-w-0 grid-cols-[1fr_auto] gap-3 border-b border-border/50 px-3 py-3 last:border-b-0">
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {media.map((item) => (
                    <figure key={item.id} className="overflow-hidden rounded-xl bg-surface">
                      ...
                      />
                      {item.caption && (
                        <figcaption className="p-3 text-[10px] leading-relaxed text-muted-foreground">
                          {item.caption}
                        </figcaption>
                      )}
                    </figure>
              ...
              {character.tags.length > 0 && (
                <div className="mt-4 flex flex-wrap gap-2">
                  {character.tags.map((tag) => (
                    <span key={tag} className="rounded-full border border-border bg-surface px-3 py-1.5 text-[10px] font-semibold">
                      {tag}
                    </span>
                  ))}
                </div>
            ...
            <div className="glass overflow-hidden">
              ...
              <div className="divide-y divide-border/50 px-4">
                {infoRows.map(([label, value]) =>
                  value ? (
                    <div key={label} className="grid grid-cols-[92px_minmax(0,1fr)] gap-3 py-3 text-xs">
