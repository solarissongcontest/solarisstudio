      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_21rem]">
        <div className="min-w-0 space-y-4">
          {data.internal_entry ? (
            <AdminCard>
              <AdminCardHeader
                ...
                description="Accept the entry when it is valid. Declines require a reason and confirmation."
              />
          ...
          {!data.internal_entry && !data.national_final ? (
            <AdminCard>
              <p className="text-sm text-muted-foreground">No entry information has been submitted for review yet.</p>
            </AdminCard>
          ) : null}
  ...
  return (
    <article className={`rounded-xl border p-3.5 ${winner ? "border-amber-200/20 bg-amber-200/[0.045]" : "border-white/[0.07] bg-white/[0.018]"}`}>
      ...
      <AdminConfirmSheet
        ...
        busy={busy}
        onConfirm={async () => {
          if (pendingStatus) await review(pendingStatus);
        }}
      />
    </article>
  );
