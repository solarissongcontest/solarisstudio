  ...
  return (
    <AppShell>
      <PageHeader
        ...
        description={
          selectedEdition
            ? `Operational deadlines for ${editionLabel(selectedEdition)}, plus the organizer change history.`
      ...
      <div className="grid min-w-0 gap-4 xl:grid-cols-[.85fr_1.15fr]">
        <div className="space-y-4">
          ...
          >
            <form onSubmit={submit} className="space-y-3">
              ...
              <button
                disabled={createDeadline.isPending || !selectedEdition}
          ...
          <Panel
            title={selectedEdition ? `${editionLabel(selectedEdition)} deadlines` : "Deadlines"}
            description="Click a deadline to mark it complete or reopen it."
          >
            <div className="space-y-2">
              {(deadlines ?? []).map((item) => (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    toggleDeadline.mutate({ id: item.id, complete: !item.completed_at })
                  }
                  className="flex w-full items-start gap-3 rounded-xl border border-border bg-surface p-3 text-left"
                >
                  <span
                    className={`mt-1 h-3 w-3 shrink-0 rounded-full ${
                      item.completed_at ? "bg-emerald-400" : "bg-amber-300"
                    }`}
                  />
                  <span className="min-w-0">
                    <span className="block text-xs font-bold">{item.label}</span>
                    <span className="mt-1 block text-[10px] text-muted-foreground">
                      {new Date(item.due_at).toLocaleString()} · {item.kind}
                    </span>
                    {item.notes && (
                      <span className="mt-1 block text-[10px] text-muted-foreground">{item.notes}</span>
                    )}
                  </span>
                </button>
              ))}
              ...
              {!(deadlines ?? []).length && (
                <p className="text-sm text-muted-foreground">No deadlines for this edition.</p>
              )}
            </div>
          </Panel>
        </div>
        ...
        <Panel
          title="Audit log"
          description="Recent important changes across contest and country data."
        >
          <div className="divide-y divide-border/55">
            ...
            {!(audit ?? []).length && (
              <p className="text-sm text-muted-foreground">No audit rows available yet.</p>
            )}
          </div>
        </Panel>
      </div>
