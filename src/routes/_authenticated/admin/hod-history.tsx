...
import { ArrowLeft, History, Lightbulb, Plus, Trash2, UserRoundCog } from "lucide-react";
  ...
  return (
    <AdminPage>
      <AdminPageHeader
        ...
        title="HOD history"
        description="Track the real person controlling each delegation by edition. The country remains the official contest identity; this history improves attribution in analytics and integrity review."
      ...
      {data ? (
        <>
          <div id="hod-assignment" className="grid gap-4 xl:grid-cols-[.8fr_1.2fr]">
            <AdminCard>
              <AdminCardHeader eyebrow="People" title="HOD identities" description="Create one identity per real person and reuse it across every edition they controlled." />
            ...
            <AdminCard>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <Field label="Notes"><input value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Optional historical note" className={controlClass} /></Field>
              </div>
              <div className="mt-4 rounded-xl border border-white/[0.07] bg-white/[0.025] p-3 text-xs leading-relaxed text-muted-foreground">This will assign {selectedEditionIds.length} edition{selectedEditionIds.length === 1 ? "" : "s"}. An existing assignment for the same country, edition and channel is replaced.</div>
                  ...
                  <div key={`${suggestion.editionId}:${suggestion.countryId}`} className="py-3 first:pt-0 last:pb-0">
                    <div className="flex min-w-0 items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 text-xs leading-relaxed text-muted-foreground">{suggestion.dominantBallots}/{suggestion.totalBallots} ballots · {suggestion.evidenceRounds} round{suggestion.evidenceRounds === 1 ? "" : "s"} · username evidence</p>
          ...
          <AdminCard className="!p-0 overflow-hidden">
            <div className="p-4 sm:p-5"><AdminCardHeader eyebrow="History" title="Assignment history" description="Channel overrides take precedence over the delegation default only for that edition." /></div>
            ...
            <div className="divide-y divide-white/[0.07] p-4 sm:p-5">
              {filteredAssignments.map((assignment) => (
                <div key={assignment.id} className="py-3 first:pt-0 last:pb-0">
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1"><div className="flex min-w-0 flex-wrap items-center gap-2"><p className="truncate text-sm font-semibold text-foreground">{assignment.countryName} · {assignment.personName}</p><AdminStatus tone="neutral">{assignment.channel === "delegation" ? "Jury + televote" : `${assignment.channel} override`}</AdminStatus></div><p className="mt-1 truncate text-xs text-muted-foreground">{assignment.countryCode} · {assignment.identityKey}</p></div>
                    <button type="button" onClick={() => setRemoveTarget(assignment as Assignment)} className="admin-action-danger !min-h-10 !px-3" aria-label={`Remove ${assignment.personName} assignment for ${assignment.countryName}`}><Trash2 className="size-4" /></button>
                  </div>
                </div>
              ))}
