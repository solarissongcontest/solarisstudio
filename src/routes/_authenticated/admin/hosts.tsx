...
export const Route = createFileRoute("/_authenticated/admin/hosts")({
  head: () => ({
    meta: [{ title: "Host settings — Solaris Studio" }],
  }),
...
function HostSettingsPage() {
  ...
  const editionShows = useMemo(
    () =>
      ((shows ?? []) as HostedShow[])
        .filter((show) => show.edition_id === selectedEdition?.id)
        .sort((a, b) => a.sort_order - b.sort_order),
    [shows, selectedEdition?.id],
  );

  const draftFor = (show: HostedShow): Draft =>
    drafts[show.id] ?? {
  ...
  const saveShow = async (show: HostedShow) => {
    ...
    try {
      const { error: saveError } = await (supabase.from("shows") as any)
        .update({
  ...
  const copyToAll = async (source: HostedShow) => {
    ...
    if (!window.confirm(`Use ${draft.city || "this city"} as the host for every show in ${editionLabel(selectedEdition)}?`)) {
      return;
    }
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        title="Host settings"
        description="Assign a host country and city to each show. This supports split-host editions where the semi-finals and Grand Final take place in different countries or cities."
      />
      ...
      <Panel title="Edition" description="Choose the edition whose show locations you want to manage.">
        <Select
        ...
        {editionShows.map((show) => {
          ...
          return (
            <Panel
              key={show.id}
              title={show.name}
              description={`${show.kind.replaceAll("-", " ")} · show ${show.sort_order}`}
            >
              <div className="grid min-w-0 gap-3 md:grid-cols-2">
                <Field label="Host country">
                  <Select
                    value={draft.countryId}
                    onChange={(event) => setDraft(show, { countryId: event.target.value })}
                  >
                ...
                <Field label="Host city">
                  <TextInput
                    ...
                    onChange={(event) => setDraft(show, { city: event.target.value })}
                  />
                </Field>
              </div>
              ...
              <div className="mt-4 flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <p className="min-w-0 text-xs text-muted-foreground">
                  {selectedCountry?.name || "Country TBC"}
                  {draft.city ? ` · ${draft.city}` : " · City TBC"}
                </p>
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    onClick={() => void saveShow(show)}
                    className="min-h-10 rounded-xl border border-border bg-surface px-3 text-xs font-semibold disabled:opacity-50"
                  >
                    {savingId === show.id ? "Saving…" : "Save host"}
                  </button>
                  <button
                    type="button"
                    disabled={savingId != null}
                    onClick={() => void copyToAll(show)}
                    className="min-h-10 rounded-xl border border-border bg-surface px-3 text-xs font-semibold disabled:opacity-50"
                  >
                    Use for every show
                  </button>
        ...
        {selectedEdition && editionShows.length === 0 && (
          <Panel>
            <p className="text-sm text-muted-foreground">
              Create the shows for {editionLabel(selectedEdition)} first, then their host locations can be assigned here.
            </p>
          </Panel>
        )}
      </div>
