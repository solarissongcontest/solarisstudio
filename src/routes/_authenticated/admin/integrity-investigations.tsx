...
import { AlertTriangle, ArrowRight, Search, SearchCheck, ShieldCheck } from "lucide-react";
...
export function IntegrityInvestigations() {
  ...
  return <AdminPage><div className="mx-auto max-w-[1480px]"><AdminPageHeader eyebrow="Trust & Integrity" title="Investigation desk" description="Triage cases, assign reviewers, examine evidence, link exact rules, record findings and publish anonymised precedent without mixing allegations with verdicts." actions={<Link to="/integrity" target="_blank" className="admin-action-secondary"><ShieldCheck className="size-4"/>Reporter view</Link>}/>
    <div className="grid gap-3 sm:grid-cols-4"><Metric value={cases.filter((item) => !item.status.startsWith("closed")).length} label="Open"/><Metric value={cases.filter((item) => item.priority === "urgent" && !item.status.startsWith("closed")).length} label="Urgent" danger/><Metric value={cases.filter((item) => item.status === "waiting_for_reporter").length} label="Waiting for reporter"/><Metric value={cases.filter((item) => item.status === "investigation_opened").length} label="Formal investigations"/></div>
    <AdminCard className="mt-4"><div className="flex flex-wrap items-center justify-between gap-3"><label className="flex min-h-10 min-w-[18rem] flex-1 items-center gap-2 rounded-xl border border-white/[0.08] bg-black/10 px-3"><Search className="size-4 text-muted-foreground"/><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search case, country, category or edition…" className="min-w-0 flex-1 border-0 !bg-transparent text-xs shadow-none outline-none focus-visible:!shadow-none"/></label><div className="flex gap-1.5">{(["open","urgent","waiting","all"] as const).map((item) => <button key={item} type="button" onClick={() => setFilter(item)} className={cn("rounded-lg border px-3 py-2 text-[10px] font-bold capitalize", filter === item ? "border-sky-200/20 bg-sky-200/[0.08] text-sky-100" : "border-white/[0.07] text-muted-foreground")}>{item}</button>)}</div></div>
      <div className="mt-4 grid gap-3 lg:grid-cols-2">{filtered.map((item) => { const category = getIntegrityCategory(item.category); const status = formatIntegrityStatus(item.status); return <Link key={item.id} to="/admin/integrity-case/$caseId" params={{ caseId: item.id }} className="group rounded-[1.3rem] border border-white/[0.08] bg-white/[0.02] p-4 transition hover:border-sky-200/18 hover:bg-sky-200/[0.035]"><div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap gap-2"><span className="font-mono text-[10px] font-black text-sky-200">{item.public_code}</span><span className="text-[9px] uppercase text-muted-foreground">{item.identity_mode}</span><span className="text-[9px] uppercase text-muted-foreground">{item.case_kind.replaceAll("_"," ")}</span></div><h2 className="mt-2 text-sm font-black">{item.summary}</h2><p className="mt-1 text-[10px] text-muted-foreground">{category.label} · {status.label} · updated {new Date(item.updated_at).toLocaleString()}</p></div><div className="flex shrink-0 items-center gap-2">{item.priority === "urgent" ? <AlertTriangle className="size-4 text-rose-200"/> : null}<ArrowRight className="size-4 text-muted-foreground transition group-hover:translate-x-1"/></div></div></Link>; })}</div>
