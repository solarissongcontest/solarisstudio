...
export const Route = createFileRoute("/_authenticated/admin/countries/$countryId")({
  ...
  head: () => ({
    meta: [{ title: "Country detail — Solaris Organizer" }, { name: "robots", content: "noindex" }],
  }),
...
function CountryDetailPage() {
  ...
  const selectedEdition =
    ...
    null;
  const country =
    (countriesQuery.data ?? []).find((candidate) => candidate.id === countryId) ?? null;
        ...
        {!selectedEdition && !editionsQuery.isLoading ? (
          <AdminCard>
            <AdminEmptyState
              ...
              description="Select an edition before inspecting a delegation."
            />
          </AdminCard>
        ) : workspaceQuery.isLoading || editionsQuery.isLoading || countriesQuery.isLoading ? (
          <AdminCard>
            <p className="py-12 text-center text-sm text-muted-foreground">
              Loading country operations…
            </p>
          </AdminCard>
        ) : error ? (
          <AdminCard>
            <AdminEmptyState
              icon={AlertTriangle}
              title="Country detail could not load"
          ...
          <AdminCard>
            <AdminEmptyState
              ...
              description="The selected country is not available in this organizer context."
            />
          </AdminCard>
        ) : (
          <CountryTabContent
...
function CountryTabContent({
    ...
    return (
      <>
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <Metric
            label="Readiness"
            value={`${snapshot.operationalReadiness.score}%`}
            tone={readinessTone(snapshot.operationalReadiness.state)}
          />
          <Metric
            label="Confirmation"
            value={snapshot.context.confirmationComplete ? "Complete" : "Required"}
            tone={snapshot.context.confirmationComplete ? "ready" : "blocked"}
          />
          <Metric
            label="Entry workflow"
            value={`${snapshot.workflow.progress}%`}
            tone={
              snapshot.workflow.complete
                ? "ready"
                : snapshot.workflow.blockedCount
                  ? "blocked"
                  : "attention"
            }
          />
          <Metric
            label="HOD jury"
            value={snapshot.model.jury.complete ? "HOD assigned" : "HOD missing"}
            tone={snapshot.model.jury.complete ? "ready" : "attention"}
          />
        </section>
        <AdminCard>
  ...
  if (tab === "entry") {
    return (
      <AdminCard>
        <SectionHeading
          title="Entry"
          href="/confirmations/admin/countries"
          action="Open confirmation operations"
        />
        {snapshot.context.entry ? (
          ...
          <p className="mt-4 text-sm text-muted-foreground">
            No canonical entry is linked to this delegation.
          </p>
        )}
      </AdminCard>
    );
    ...
    return (
      <AdminCard>
        <SectionHeading
          ...
          action="Open workflow operations"
        />
  ...
  if (tab === "assets") {
    const mediaSignal = snapshot.operationalReadiness.signals.find(
      (signal) => signal.id === "media",
    );
    return (
      <AdminCard>
        <SectionHeading
          title="Assets"
          href={editionSlug ? `/admin/${editionSlug}` : "/admin"}
          action="Open contest workspace"
        />
        <div className="mt-4 rounded-xl border border-white/[0.07] bg-white/[0.02] p-4">
          <div className="flex items-center justify-between gap-2">
            <p className="font-semibold">Required entry media</p>
            <AdminStatus tone={mediaSignal?.state ?? "neutral"}>
              {mediaSignal?.state ?? "unknown"}
            </AdminStatus>
          </div>
          <p className="mt-2 text-sm text-muted-foreground">
            {mediaSignal?.message ?? "Media state is unavailable."}
          </p>
          {snapshot.context.entry?.songUrl ? (
  ...
  if (tab === "communications") {
    return (
      <AdminCard>
        ...
        />
        <p className="mt-4 text-sm text-muted-foreground">
          {snapshot.context.notices.length} published notice
          {snapshot.context.notices.length === 1 ? "" : "s"} visible in this context;{" "}
          {snapshot.model.outstandingAcknowledgements} acknowledgement
          {snapshot.model.outstandingAcknowledgements === 1 ? "" : "s"} outstanding.
        </p>
      </AdminCard>
    );
  }
  ...
  if (tab === "voting") {
    return (
      <AdminCard>
        <SectionHeading
          title="Voting"
          href={editionSlug ? `/admin/jury/${editionSlug}` : "/televoting/admin"}
          action="Open voting operations"
        />
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          ...
          <Info
            label="Ballot"
            value={snapshot.model.jury.ballotSubmitted ? "Submitted" : "Pending"}
          />
        </div>
      </AdminCard>
    );
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  {task.blockers.length
                    ? `${task.blockers.length} prerequisite blocker${task.blockers.length === 1 ? "" : "s"}`
  ...
  if (tab === "incidents") {
    return (
      <AdminCard>
        ...
        <p className="mt-4 text-sm text-muted-foreground">
          {snapshot.context.unresolvedOrganizerIssues} unresolved delegation/publication incident
          {snapshot.context.unresolvedOrganizerIssues === 1 ? "" : "s"} currently affect the
          selected edition.
        </p>
      </AdminCard>
    );
  ...
  return (
    <AdminCard>
      ...
      {snapshot.context.reviewHistory.length ? (
        <div className="mt-4 space-y-2">
          {snapshot.context.reviewHistory.map((item) => (
            ...
            >
                ...
                <AdminStatus
                  tone={
                    item.action === "accepted"
                      ? "ready"
                      : item.action === "declined" || item.action === "removed"
                        ? "blocked"
                        : "attention"
                  }
                >
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                {formatDate(item.createdAt)} · {item.targetType.replace(/_/g, " ")}
              </p>
              {item.reason ? (
        ...
        <p className="mt-4 text-sm text-muted-foreground">
          No canonical submission review history is recorded for this country and edition.
        </p>
      )}
    </AdminCard>
  );
...
function SectionHeading({ title, href, action }: { title: string; href: string; action: string }) {
  return (
    <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      <a href={href} className="admin-action-secondary">
        {action}
        <ExternalLink className="size-4" />
      </a>
    </div>
...
function errorText(error: unknown) {
  return error instanceof Error
    ? error.message
    : "The country detail cockpit could not complete that request.";
}
