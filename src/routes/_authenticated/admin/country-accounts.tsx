...
function CountryAccountsAdminPage() {
  ...
  const suspended = accountsReady
    ? data!.accounts.filter((account) => account.status === "suspended").length
    : null;
  ...
  return (
    <AdminPage>
      <AdminPageHeader
        eyebrow="Country access"
        title="Country accounts"
        description="See who can sign in for each country. You can temporarily stop an account from editing its country page if needed."
        actions={
          <Link to="/admin/more" className="admin-action-secondary">
      ...
      {message ? (
        <div className="rounded-xl border border-white/[0.08] bg-white/[0.035] p-3 text-sm text-foreground">
          {message}
        </div>
      ) : null}

      <div className="grid grid-cols-3 gap-2">
        <Metric label="Has account" value={claimed ?? "—"} />
        <Metric label="No account" value={unclaimed ?? "—"} />
        <Metric label="Editing stopped" value={suspended ?? "—"} attention={(suspended ?? 0) > 0} />
      </div>

      <AdminCard>
        <AdminCardHeader
          eyebrow="Countries"
          title="Who controls each country?"
          description="Search by country, country code or account email. Open an account to change its editing access."
          action={
            accountsFetching && !accountsLoading ? (
              <AdminStatus tone="neutral">Refreshing…</AdminStatus>
            ) : undefined
          }
        />

        {accountsLoading ? (
          <div className="py-10 text-center text-sm text-muted-foreground">Loading country accounts…</div>
        ) : accountsFailed ? (
          <AdminEmptyState
            icon={RefreshCw}
            title="Country accounts could not be loaded"
            description={accountsError instanceof Error ? accountsError.message : "Try again."}
            action={
              <button
                type="button"
          ...
          <AdminEmptyState
            icon={ShieldCheck}
            title="Account controls are unavailable"
            description="Country account controls are temporarily unavailable. Try again later."
          />
        ) : (
          <>
            <input
              ...
              placeholder="Search country, code or email…"
              className="mb-4 min-h-11 w-full rounded-xl border border-white/[0.1] bg-white/[0.035] px-3 text-sm text-foreground outline-none focus:border-sky-200/30"
            />
                  ...
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex min-w-0 flex-wrap items-center gap-2">
                        ...
                        <AdminStatus
                          tone={
                            !account ? "neutral" : account.status === "suspended" ? "blocked" : "ready"
                          }
                        >
                          {!account
                            ? "No account"
                      ...
                      <p className="mt-1 truncate text-xs text-muted-foreground">
                        {account?.email ?? country.short_code}
                      </p>
                    </div>
                    <div className="flex shrink-0 gap-1.5">
                      <Link
                        ...
                        className="admin-action-secondary !min-h-10 !px-3"
                      >
                        Open
                      </Link>
                      {account ? (
                        <button
                          ...
                          className="admin-action-secondary !min-h-10 !px-3"
                          aria-label={`Manage ${country.name} account`}
                        >
      ...
      <AdminSheet
        open={!!target}
        onClose={() => !setStatus.isPending && setTarget(null)}
        title={target ? `${target.country_name} account` : "Country account"}
        description={target?.email ?? "Account settings"}
      >
        {target ? (
          <div className="space-y-4">
            <div className="rounded-xl border border-white/[0.07] bg-white/[0.025] p-3">
              <div className="flex items-center justify-between gap-3">
                <p className="text-sm font-semibold text-foreground">Can edit country page</p>
                <AdminStatus tone={target.status === "suspended" ? "blocked" : "ready"}>
                  {target.status === "suspended" ? "No" : "Yes"}
                </AdminStatus>
              </div>
              <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
                Stopping editing prevents this account from changing its country page. The public country page and contest results are not removed.
              </p>
            </div>
            ...
            {target.status === "active" ? (
              <>
                <label className="block">
                  <span className="admin-section-label">Reason shown to the account owner</span>
                  <textarea
                    value={reason}
                ...
                >
                  {setStatus.isPending ? "Stopping editing…" : "Stop editing access"}
                </button>
              </>
            ) : (
              <>
                <div className="rounded-xl border border-white/[0.07] bg-white/[0.025] p-3 text-sm text-muted-foreground">
                  {target.suspension_reason || "No reason was provided."}
                </div>
                <button
                  type="button"
                  disabled={setStatus.isPending}
                ...
                >
                  {setStatus.isPending ? "Restoring editing…" : "Allow editing again"}
                </button>
              </>
            )}
          </div>
