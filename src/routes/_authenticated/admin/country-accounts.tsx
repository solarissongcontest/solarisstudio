  ...
  return (
    <AdminPage>
      ...
      <AdminCard>
        ...
        {accountsLoading ? (
          ...
          <AdminEmptyState
            ...
            description={
              accountsError instanceof Error
                ...
                : "The ownership directory request failed. No claim totals are being guessed."
            }
            action={
              <button
                type="button"
          ...
          <AdminEmptyState
            ...
            description="Country account moderation is temporarily unavailable."
          />
        ) : (
          <>
            <input
      ...
      <AdminSheet
        ...
        description={target?.email ?? "Account moderation"}
      >
        {target ? (
          <div className="space-y-4">
            ...
            <div className="rounded-xl border border-white/[0.07] bg-white/[0.025] p-3">
              <div className="flex items-center justify-between gap-3">
                <p className="text-sm font-semibold text-foreground">Editing access</p>
                <AdminStatus tone={target.status === "suspended" ? "blocked" : "ready"}>
                  {target.status === "suspended" ? "Suspended" : "Active"}
                </AdminStatus>
              </div>
              <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
                Suspending an account blocks country-profile editing at the database level. It does
                not remove the public country or affect contest results.
              </p>
            </div>
            ...
            {target.status === "active" ? (
              <>
                <label className="block">
                  <span className="admin-section-label">Reason shown to owner</span>
                  <textarea
                    value={reason}
                ...
                >
                  {setStatus.isPending ? "Suspending…" : "Suspend editing access"}
                </button>
              </>
            ) : (
              <>
                <div className="rounded-xl border border-white/[0.07] bg-white/[0.025] p-3 text-sm text-muted-foreground">
                  {target.suspension_reason || "No suspension reason was provided."}
                </div>
                <button
                  type="button"
                  disabled={setStatus.isPending}
                ...
                >
                  {setStatus.isPending ? "Restoring…" : "Restore editing access"}
                </button>
              </>
            )}
          </div>
...
function Metric({
  ...
  return (
    <div
      className={`admin-card px-3 py-3 text-center ${attention ? "!border-rose-200/15 !bg-rose-200/[0.045]" : ""}`}
    >
