...
function AdminEdition() {
  ...
  const [scoreboard, setScoreboard] = useState<ScoreboardConfig>(
    resolveScoreboard(activeShow?.broadcast_config, {
      theme: savedTheme,
  ...
  const broadcastRows = useMemo<BroadcastRowData[]>(
    () =>
      standings.map((standing, index) => {
        const display = eMap.get(standing.countryId);
        const participant = pMap.get(standing.countryId);
        const qualified = participant?.qualified ?? null;

        return {
          id: standing.countryId,
          entityType: display?.entityType ?? "global",
          name: display?.name ?? standing.countryId,
          abbreviation: display?.short_code ?? "",
          flagImage: display?.flag_image ?? null,
          accent: display?.accent_color ?? savedTheme.colors.accent,
          rank: standing.rank,
          runningOrder: participant?.running_order ?? null,
    ...
    [standings, eMap, pMap, savedTheme.colors.accent],
  );
  ...
  const saveBroadcast = () => {
    ...
    const existing =
      activeShow.broadcast_config && typeof activeShow.broadcast_config === "object"
    ...
    return patchShow(
      activeShow,
      { broadcast_config: nextBroadcastConfig },
  ...
  const resetScoreboard = () => {
    ...
    const automatic = resolveScoreboard(null, {
      theme: themeDraft,
      rowCount: order.length,
    });

    setScoreboard(automatic);
    setMsg(
      ...
      <div className="grid gap-6 lg:grid-cols-[1.5fr_1fr]">
          ...
          {tab === "Broadcast" && activeShow && (
            <div className="space-y-6">
              ...
              <Panel
                ...
                description="Scenes, timing, reveal pacing and effects"
              >
        ...
        <div className="space-y-4">
          <Panel
            ...
            description={
              tab === "Broadcast"
                ? "Current broadcast scoreboard draft"
                : "Current show scoreboard"
            }
          >
            {tab === "Broadcast" ? (
              <div className="overflow-hidden rounded-2xl border border-border bg-black">
                    ...
                    <ScoreboardBoard
                      ...
                      panelContent={
                        <div className="flex h-full flex-col items-center justify-center text-center">
                          <div className="grid aspect-[3/2] w-full max-w-52 place-items-center rounded-xl bg-primary/20 text-3xl font-black">
                            OLA
                          </div>
                ...
                {!broadcastRows.length && (
                  <p className="p-6 text-center text-sm text-white/60">
                    Add entries to preview the scoreboard.
                  </p>
                )}
              </div>
            ) : (
