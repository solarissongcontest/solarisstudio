  ...
  head: () => ({
    meta: [
      { title: "Encyclopedia — Solaris Studio" },
      {
        name: "description",
        content:
          "Published Solaris Song Contest history: editions, countries, entries, artists, shows and results.",
      },
    ],
    links: [{ rel: "canonical", href: "https://studio.solaris-song-contest.workers.dev/encyclopedia" }],
  }),
...
function EncyclopediaPage() {
  ...
  const items = useMemo<EncyclopediaItem[]>(() => {
    ...
    for (const edition of publicArchive.editions) {
      output.push({
        ...
        search: [edition.name, edition.edition_number, edition.year, edition.host_city].filter(Boolean).join(" "),
      });
    }

    for (const country of countries) {
      const hasPublicHistory =
        publicArchive.participants.some((row) => row.country_id === country.id) ||
        publicArchive.results.some((row) => row.country_id === country.id);
      ...
      output.push({
        ...
        search: [country.name, country.native_name, country.short_code, country.region].filter(Boolean).join(" "),
        flag: { code: country.short_code, color: country.accent_color, image: country.flag_image },
      });
    }
    ...
    for (const participant of publicArchive.participants) {
      ...
      if (!seenEntry.has(entryKey)) {
        ...
        const title = [participant.artist, participant.song].filter(Boolean).join(" — ") || `${country.name} entry`;
        output.push({
          id: `entry:${entryKey}`,
          kind: "entry",
          title,
          ...
          href: `/countries/${country.short_code}`,
          search: [title, country.name, country.short_code, edition.name, edition.edition_number].join(" "),
          flag: { code: country.short_code, color: country.accent_color, image: country.flag_image },
        });
      }
      if (participant.artist) {
        ...
        artistCounts.set(key, { name: participant.artist.trim(), count: (current?.count ?? 0) + 1 });
      }
    }
    ...
    for (const [key, artist] of artistCounts) {
      output.push({
        ...
        kind: "artist",
        title: artist.name,
        subtitle: artist.count === 1 ? "1 published SSC entry" : `${artist.count} published SSC entries`,
    ...
    for (const show of publicArchive.shows) {
      ...
      output.push({
        ...
        href: `/shows/${show.id}`,
        search: [show.name, show.kind, edition?.name, edition?.edition_number].filter(Boolean).join(" "),
      });
    }
  ...
  const filtered = useMemo(() => {
    ...
    return items
      ...
      .filter((item) => {
        ...
        const haystack = `${item.title} ${item.subtitle} ${item.search}`.toLocaleLowerCase();
  ...
  if (feature.isLoading || loading) {
    return (
      <AppShell>
        <PageHeader eyebrow="Explore Solaris" title="Encyclopedia" description="Published Solaris history, connected." />
      ...
      <AppShell>
        ...
        <Panel title="Encyclopedia is not enabled yet">
          <p className="text-sm text-muted-foreground">
            The product is installed but its rollout flag is still off while production verification finishes.
          </p>
        </Panel>
      </AppShell>
    );
  ...
  if (error) return <AppShell><ArchiveDataError /></AppShell>;
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        title="Encyclopedia"
        description="A publication-safe reference across editions, countries, entries, artists, shows and official results."
        actions={<Link to="/wiki" className="rounded-xl border border-border bg-surface px-3 py-2 text-sm font-semibold">Country Wiki →</Link>}
      />

      <Panel title="Archive coverage" description="Only information already released on public Solaris surfaces is indexed here.">
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <StatTile label="Editions" value={publicArchive.editions.length} />
          <StatTile label="Countries" value={new Set(publicArchive.participants.map((row) => row.country_id)).size} />
          <StatTile label="Published entries" value={new Set(publicArchive.participants.map((row) => `${row.edition_id}:${row.country_id}`)).size} />
      ...
      <section className="mt-5 rounded-2xl border border-border bg-surface/60 p-3 sm:p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-1.5" aria-label="Filter Encyclopedia">
          {(["all", "edition", "country", "entry", "artist", "show"] as Kind[]).map((value) => (
            <button
              key={value}
              type="button"
              onClick={() => setKind(value)}
              aria-pressed={kind === value}
              className={`min-h-9 rounded-lg px-3 text-xs font-semibold ${kind === value ? "bg-primary/12 text-primary" : "bg-background/45 text-muted-foreground"}`}
            >
      ...
      <section className="mt-5">
        <div className="mb-3 flex items-end justify-between gap-3 border-b border-border/60 pb-3">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-primary">Reference index</p>
        ...
        <div className="grid gap-2 md:grid-cols-2 lg:grid-cols-3">
          {filtered.map((item) => (
            <Link key={item.id} to={item.href as any} className="flex min-w-0 items-center gap-3 rounded-2xl border border-border bg-surface p-4 transition hover:border-primary/35">
              {item.flag ? <FlagChip code={item.flag.code} color={item.flag.color} image={item.flag.image} size="md" /> : null}
              <div className="min-w-0 flex-1">
                <p className="text-[9px] font-bold uppercase tracking-[0.14em] text-primary">{item.kind}</p>
                <p className="mt-1 truncate text-sm font-semibold">{item.title}</p>
                <p className="mt-0.5 truncate text-xs text-muted-foreground">{item.subtitle || "Published Solaris record"}</p>
              </div>
      ...
      <p className="mt-5 text-xs leading-relaxed text-muted-foreground">
        Historical gaps are left as gaps. Draft entries, organizer notes, private ballots and unpublished results are never indexed here.
      </p>
    </AppShell>
  );
}
