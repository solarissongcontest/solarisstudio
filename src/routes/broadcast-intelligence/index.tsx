...
export const Route = createFileRoute("/broadcast-intelligence/")({
  head: () => ({
    meta: [
      {
        title: "Broadcast Intelligence — Solaris Studio",
      },
    ],
  }),
  component: BroadcastIntelligencePage,
});

const MOMENT_LABELS: Record<BroadcastMomentKind, string> = {
...
function BroadcastIntelligencePage() {
  ...
  const entries = useMemo(
    () =>
      broadcastEntriesFromResults(
        results ?? [],
        (id) => displayMap.get(id)?.name ?? "Unknown entry",
      ),
    [results, displayMap],
  );

  const intelligence = useMemo(() => buildBroadcastIntelligence(entries), [entries]);

  const liveRows = useMemo(() => replayProgress(entries, replayStep), [entries, replayStep]);
  ...
  const currentReveal = replayStep > 0 ? intelligence.replay[replayStep - 1] : null;

  useEffect(() => {
    if (!autoPlay || replayStep >= intelligence.replay.length) {
      if (replayStep >= intelligence.replay.length) {
        setAutoPlay(false);
      }
    ...
    const timer = window.setTimeout(() => {
      setReplayStep((current) => Math.min(intelligence.replay.length, current + 1));
    }, 1400);

    return () => window.clearTimeout(timer);
  }, [autoPlay, replayStep, intelligence.replay.length]);

  const nextReveal = intelligence.replay[replayStep];

  const replayFinished = intelligence.replay.length > 0 && replayStep >= intelligence.replay.length;

  const setEdition = (value: string) => {
    setEditionId(value);
    setShowId("");
  };
    ...
    <AppShell>
      ...
      <div className="grid min-w-0 gap-4 lg:grid-cols-[320px_minmax(0,1fr)] lg:gap-5">
        <div className="min-w-0 space-y-4">
          <Panel title="Choose a result" description="Only fully published results are available">
            <label
              htmlFor="broadcast-edition"
            ...
            >
              {!eligibleShows.length && <option value="">No published result</option>}
          ...
          {intelligence.winner && (
            <Panel title="Official winner" description={selectedShow?.name}>
              <div className="flex min-w-0 items-center gap-3">
                {(() => {
                  const display = displayMap.get(intelligence.winner!.id);
                ...
                <div className="min-w-0">
                  <p className="truncate font-display text-xl font-semibold">
                    {intelligence.winner.name}
                  </p>

                  <p className="mt-1 text-xs text-muted-foreground">
                    {intelligence.winner.totalPoints} points
                  </p>
                </div>
              </div>
            </Panel>
            ...
            <Panel title="Broadcast pulse">
              <div className="grid min-w-0 grid-cols-2 gap-2 lg:grid-cols-1">
                {intelligence.metrics.map((metric) => (
                  <div key={metric.label} className="min-w-0 rounded-xl bg-surface p-3">
          ...
          {intelligence.rows.length > 0 ? (
            <>
              ...
              >
                    ...
                    {currentReveal ? (
                      <div className="mt-2 min-w-0">
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">
                          +{currentReveal.televotePoints} televote points
                        </p>

                        <p className="mt-2 break-words text-xs text-muted-foreground">
                          Now{" "}
                          <strong className="text-foreground">#{currentReveal.rankAfter}</strong>
                          {currentReveal.rankChange > 0 && ` · up ${currentReveal.rankChange}`}
                          {currentReveal.rankChange < 0 &&
                      ...
                      <div className="mt-2">
                        <p className="font-display text-xl font-semibold">Jury result</p>

                        <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
                          Start the replay to reveal the televote.
                        </p>
                      </div>
                    )}
                  </div>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setReplayStep((current) =>
                          Math.min(intelligence.replay.length, current + 1),
                        );
                      }}
                      disabled={!intelligence.replay.length}
                      className="min-h-11 min-w-0 rounded-xl border border-border bg-surface px-3 text-xs font-semibold disabled:opacity-50"
                    >
                      {replayFinished ? "Restart" : "Next reveal"}
                    </button>

                    <button
                      type="button"
                      onClick={() => setAutoPlay((current) => !current)}
                      disabled={replayFinished || !intelligence.replay.length}
                      className="min-h-11 min-w-0 rounded-xl border border-border bg-surface px-3 text-xs font-semibold disabled:opacity-50"
                    >
                ...
                <div className="mt-4">
                  <div className="flex items-center justify-between gap-3 text-[10px] text-muted-foreground">
                    <span>
                      {replayStep}/{intelligence.replay.length} revealed
                    </span>

                    {nextReveal && (
                      <span className="min-w-0 truncate text-right">Next: {nextReveal.name}</span>
                    )}
                  </div>
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-[10px] text-muted-foreground">
                            {row.televoteRevealed ? "Televote revealed" : "Waiting for televote"}
                          </p>
                        </div>
              ...
              >
                <div className="grid min-w-0 gap-2 md:grid-cols-2">
                  {intelligence.moments.map((moment) => {
                    const display = moment.countryId ? displayMap.get(moment.countryId) : undefined;

                    return (
                      <div key={moment.id} className="min-w-0 rounded-xl bg-surface p-3 sm:p-4">
                        <div className="flex min-w-0 items-start gap-2">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="mt-1 break-words text-sm font-semibold">{moment.title}</p>
                      ...
                      <div key={row.id} className="min-w-0 rounded-xl bg-surface p-3">
                        ...
                        <div className="mt-3 grid grid-cols-3 gap-2 text-xs">
                          ...
                          <div>
                            ...
                            <strong>#{row.televoteRank}</strong>
                            <p className="text-[10px] text-muted-foreground">
                              {row.televotePoints} pts
                            </p>
                          </div>
                          ...
                          <div>
                            ...
                            <strong>
                              {row.juryToFinalChange > 0
                                ? `+${row.juryToFinalChange}`
                                : row.juryToFinalChange}
                            </strong>
                            <p className="text-[10px] text-muted-foreground">jury → final</p>
                          </div>
                        </div>
                      </div>
                    );
                  ...
                  <table className="w-full min-w-[680px] text-left text-sm">
                    <thead>
                      <tr className="border-b border-border text-[10px] uppercase tracking-[0.12em] text-muted-foreground">
                        <th className="pb-3 pr-3">Final</th>

                        <th className="pb-3 pr-3">Entry</th>

                        <th className="pb-3 pr-3 text-right">Jury</th>

                        <th className="pb-3 pr-3 text-right">Televote</th>

                        <th className="pb-3 pr-3 text-right">Total</th>

                        <th className="pb-3 text-right">Jury → final</th>
                      </tr>
                    </thead>
            ...
            <Panel title="Broadcast Intelligence">
              <p className="break-words text-sm leading-relaxed text-muted-foreground">
                Choose a published show with jury and televote results to generate the replay and
                broadcast analysis.
              </p>
            </Panel>
          )}
