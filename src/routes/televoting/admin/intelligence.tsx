...
function SettingsPanel({ settings, saving, onSave }: { settings: FriendVotingSettings; saving: boolean; onSave: (settings: FriendVotingSettings) => void }) {
  ...
  return (
    <section className="space-y-4">
      <div className="glass-strong flex flex-col gap-3 p-5 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="font-display text-3xl uppercase">Friend-voting model</h2><p className="mt-2 max-w-4xl text-sm text-muted-foreground">These settings are owned by Solaris because the model now spans canonical jury history and Televoting. Changes recalculate relationships and groups immediately; they never modify ballots or official results.</p></div><button type="button" disabled={saving} onClick={() => onSave(draft)} className="rounded-xl border border-sky-200/15 bg-sky-200/10 px-4 py-2 text-xs font-semibold text-sky-100 disabled:opacity-50">{saving ? "Saving…" : "Save & recalculate"}</button></div>
