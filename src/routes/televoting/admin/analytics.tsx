  ...
  return (
    <div className="mx-auto max-w-7xl py-4 sm:py-8">
      ...
      <header className="mb-8">
        ...
        <p className="mt-3 max-w-4xl text-sm leading-relaxed text-muted-foreground">Televoting and canonical jury history are analysed together. Country remains the official delegation identity, while the HOD lens limits behaviour to editions controlled by one real person so delegation handovers do not contaminate each other.</p>
      </header>
      ...
      {adminLoading || isLoading ? <section className="glass-strong p-8 text-center text-sm text-muted-foreground">Calculating jury, televote and HOD-aware analytics…</section> : error || !data ? <section className="glass-strong border-destructive/30 p-6 text-sm text-destructive">{error instanceof Error ? error.message : "Analytics could not be loaded."}</section> : (
        <>
          ...
          {view === "summary" ? <div className="space-y-4">
            ...
            <section className="grid gap-3 lg:grid-cols-2"><article className="glass-strong p-5"><h2 className="font-medium">HOD identity coverage</h2><p className="mt-2 text-sm text-muted-foreground">Unknown history stays visible at country level but is never silently attached to a later or earlier HOD.</p><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4"><SmallMetric label="TV assigned" value={data.hod.coverage.televoteAssignedBallots} /><SmallMetric label="TV unknown" value={data.hod.coverage.televoteUnknownBallots} /><SmallMetric label="Jury assigned" value={data.hod.coverage.juryAssignedBallots} /><SmallMetric label="Jury unknown" value={data.hod.coverage.juryUnknownBallots} /></div></article><article className="glass-strong p-5"><h2 className="font-medium">Scope coverage</h2><p className="mt-2 text-sm text-muted-foreground">{data.overview.editions} canonical edition{data.overview.editions === 1 ? "" : "s"} · {data.overview.rounds} Televoting round{data.overview.rounds === 1 ? "" : "s"} · {data.overview.juryVotes} stored jury score{data.overview.juryVotes === 1 ? "" : "s"}.</p><p className="mt-3 text-xs text-muted-foreground">Deleted Televoting ballots are excluded here but remain available to Integrity.</p></article></section>
          </div> : null}
