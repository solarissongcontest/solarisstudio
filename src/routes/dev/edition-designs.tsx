      ...
      <div className="mt-4 overflow-x-auto border border-border bg-background/40 p-2">
        ...
        >
          <div className="edition-public-page" data-edition-design-lab>
            ...
            {fixture.results ? <EditionSection id="lab-stories" eyebrow="The result" title="Stories"><p className="edition-empty-state">Archived result story fixture.</p></EditionSection> : null}
            ...
            {fixture.entries > 0 ? <EditionSection id="lab-countries" eyebrow="Delegations" title="Countries"><div className="edition-country-list">{entries.slice(0, 6).map((entry, index) => <div className="edition-country-item" key={`${entry.country}-${index}`}>{entry.country}</div>)}</div></EditionSection> : null}
          </div>
        </div>
      </div>
    </AppShell>
