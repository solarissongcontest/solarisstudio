...
export const MY_SOLARIS_NOTICE_STATES = [
  "unread",
  "read",
...
const INBOX_STATE_SET = new Set<string>(MY_SOLARIS_NOTICE_STATES);

export type MySolarisNoticesSearch = {
  notice?: string;
  state?: NoticeInboxState;
};

export function parseMySolarisNoticesSearch(search: Record<string, unknown>): MySolarisNoticesSearch {
  return {
...
export function MySolarisNoticesPage() {
    ...
    return (
      <AppShell>
        <PageHeader
          ...
          description="Official communications are currently unavailable for this account."
          actions={<BackToTasks />}
        />
      </AppShell>
    );
    ...
    return (
      <AppShell>
        <PageHeader
          ...
          description="Organizer access is intentionally separate from delegation read and acknowledgement receipts. Use Official Communications for organizer-side operations."
          actions={
            <Link
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        description="Official notices, acknowledgement requests and communication history for your delegation."
        actions={<BackToTasks />}
      />
      ...
      <div className="space-y-4">
        ...
        <Panel title="Inbox filters">
          <div className="flex flex-wrap gap-2">
            ...
            />
            {MY_SOLARIS_NOTICE_STATES.map((state) => (
              <FilterButton
                key={state}
...
function parseNoticeSearch(value: unknown): MySolarisNoticesSearch {
  if (!value || typeof value !== "object") return {};
  return parseMySolarisNoticesSearch(value as Record<string, unknown>);
}
