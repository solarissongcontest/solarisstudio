...
export const getMergedScopedAnalytics = createServerFn({ method: "POST" })
  .inputValidator((data: { scope: MergedAnalysisScope }) => ({ scope: validateScope(data.scope) }))
  .handler(async ({ data }) => {
    const { getMergedScopedAnalyticsServer } = await import(
      "@/integrations/televoting/analytics.server"
    );
    return getMergedScopedAnalyticsServer(data.scope);
  });
