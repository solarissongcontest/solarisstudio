...
export const getMergedTelevotingIntelligence = createServerFn({ method: "POST" })
  .inputValidator((data?: { lens?: IntelligenceLens; channel?: IntelligenceChannel; hodPersonId?: string | null; editionId?: string | null }) => ({
  ...
  .handler(async ({ data }) => {
    ...
    return {
      ...result,
      // Model-v2 risk components are calculated server-side from the complete
      // observation set. Do not recalculate them from rounded UI aggregates.
      stats: {
        ...result.stats,
