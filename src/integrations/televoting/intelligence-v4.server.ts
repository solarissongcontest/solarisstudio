...
function reciprocalEvidence(
  ...
  for (const row of pair) {
    ...
    const age =
      currentEdition != null && row.editionNumber != null
        ? Math.max(0, currentEdition - row.editionNumber)
        : 0;
...
export async function getMergedIntelligenceV4Server(
  ...
  const base = await getMergedIntelligenceServer({
    ...options,
  ...
  // Build the complete history first. Edition/HOD filters are applied only to
  // the relationship under review, not to its historical or field baseline.
  ...
  if (channel !== "jury") {
    for (const submission of (submissionResult.data ?? []) as Submission[]) {
      ...
      for (const entry of entriesBySubmission.get(submission.id) ?? []) {
        const code =
          entryCode.get(`${submission.round_id}:${entry.target_country_code}`) ??
          upper(entry.target_country_code);
        if (code) points.set(code, Number(entry.points ?? 0));
      }

      const targets = [
        ...(participantCodesByRound.get(submission.round_id) ?? new Set<string>()),
      ].filter((code) => code !== voterCode);
  ...
  if (channel !== "televote") {
    ...
    for (const ballot of byBallot.values()) {
      ...
      const countryFallback =
        lens === "hod" && !countriesWithHodHistory.has(countryId);
      if (lens === "hod" && !hod && !countryFallback) continue;
      const identityKey =
        lens === "country"
          ? `country:${voterCode}`
      ...
      const scores = new Map(
        ballot
          ...
          .map((vote) => [
            String(vote.receiving_country_id),
      ...
      for (const targetId of participants) {
        ...
        if (!target) continue;
        rows.push({
          target: upper(target.short_code ?? target.name),
          score: scores.get(targetId) ?? 0,
        });
      }
      const rankMap = ranks(rows);
      const maxScore = Math.max(0, ...rows.map((row) => row.score));
  ...
  // When an older edition is selected, future editions must not leak into its
  // baseline. Keep the selected edition plus all chronologically older history.
  const historicalScope = observations.filter((row) => {
  ...
  for (const row of historicalScope) {
    ...
    if (options.hodPersonId && row.controllerPersonId !== options.hodPersonId) continue;
