...
export function calculateFriendVotingRisk(input: FriendVotingRiskInput): FriendVotingRiskResult {
  ...
  // Distinct HOD-controlled editions are the independent historical sample.
  // Extra jury ballots or website rounds inside one edition do not increase it.
  const sampleConfidence = Math.min(1, uniqueEditions / 4);
