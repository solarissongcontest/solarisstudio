...
export function IntegrityCentre() {
  ...
  const openCase = (
    nextCredential: AnonymousCaseCredential,
    nextSnapshot?: AnonymousCaseSnapshot,
  ) => {
...
function IntegrityHero() {
  return (
    <section className="relative overflow-hidden rounded-[2rem] border border-emerald-300/15 bg-[linear-gradient(145deg,rgba(17,60,64,.58),rgba(5,19,42,.97)_55%,rgba(26,37,76,.96))] p-5 shadow-[0_28px_80px_rgba(0,4,28,.24)] sm:p-8 lg:p-10">
      <div
        aria-hidden="true"
        className="absolute -right-24 -top-24 size-80 rounded-full bg-emerald-300/10 blur-3xl"
      />
      <div
        aria-hidden="true"
        className="absolute -bottom-28 left-1/4 size-72 rounded-full bg-violet-400/8 blur-3xl"
      />
      <div className="relative max-w-3xl">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground">
            Protected reporting · two-way cases · fair review
          </span>
        </div>
        ...
        <p className="mt-4 max-w-2xl text-sm leading-7 text-slate-200/80 sm:text-base">
          The Integrity Centre lets you report a possible SSC rule break, continue a protected
          conversation with TSBC and add information later. A report is information for review, not
          a verdict against anyone.
        </p>
      </div>
    </section>
  );
    ...
    <div className="mt-8 space-y-8">
      <section className="grid gap-4 lg:grid-cols-3">
        ...
        >
          ...
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            Create a case that does not store your Solaris account identity. Keep a recovery key to
            return and talk with TSBC.
          </p>
          <span className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-emerald-200">
            Start securely <ArrowRight className="size-4 transition group-hover:translate-x-1" />
          </span>
        </button>
        ...
        >
          ...
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            Use your case code and recovery key to read TSBC replies and send more information.
          </p>
          <span className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-sky-200">
            Enter credentials <ArrowRight className="size-4 transition group-hover:translate-x-1" />
          </span>
        </button>
        ...
        >
          ...
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            See the official rules on anonymity, evidence, anti-retaliation and the difference
            between a report and a finding.
          </p>
          <span className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-violet-200">
            Read Rule 16.3 <ArrowRight className="size-4 transition group-hover:translate-x-1" />
          </span>
        </Link>
      </section>
      ...
      {savedCases.length ? (
        <section>
          <SectionHeading
            eyebrow="This device"
            title="Saved anonymous cases"
            description="These credentials live only in this browser's local storage. They are not attached to your Solaris account."
          />
      ...
      <section>
        <SectionHeading
          eyebrow="Principles"
          title="What this system promises"
          description="The integrity process protects both the person reporting and the person being reported."
        />
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <Principle
            icon={EyeOff}
            title="Identity minimised"
            text="Fully anonymous case records do not contain a reporter Solaris account ID."
          />
          <Principle
            icon={MessageCircle}
            title="Two-way"
            text="TSBC can ask questions and the reporter can answer without abandoning anonymity."
          />
          <Principle
            icon={Scale}
            title="No instant guilt"
            text="A report, allegation or automated flag does not itself establish misconduct."
          />
          <Principle
            icon={LockKeyhole}
            title="Need-to-know"
            text="Sensitive evidence and technical data should not be exposed unnecessarily."
          />
        </div>
      </section>
    </div>
...
function AnonymousReportFlow({
  ...
  const valid = Boolean(
    draft.category && draft.summary.trim().length >= 5 && draft.details.trim().length >= 20,
  );
  ...
  return (
    <section className="mt-8">
      ...
      >
        <ArrowLeft className="size-4" /> Integrity Centre
      </button>
        ...
        <div className="rounded-[1.7rem] border border-white/[0.08] bg-[linear-gradient(155deg,rgba(18,42,71,.82),rgba(5,18,39,.94))] p-5 sm:p-7">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <h2 className="mt-1 text-2xl font-black tracking-[-.04em] sm:text-3xl">
                {reviewing ? "Review before sending" : "What happened?"}
              </h2>
            </div>
            <span className="rounded-full border border-emerald-300/15 bg-emerald-300/[0.06] px-3 py-1 text-[10px] font-black uppercase tracking-[.1em] text-emerald-100">
          ...
          {!reviewing ? (
            <div className="mt-6 space-y-6">
              ...
              <TextField
                ...
                placeholder="Example: Possible agreement to exchange high televote scores"
                maxLength={180}
              />
              <TextArea
                label="Describe the concern"
                value={draft.details}
                onChange={(value) => setDraft({ ...draft, details: value })}
                placeholder="Explain what happened and why you think it may be relevant to SSC. You do not need to know the exact rule number."
                minRows={5}
              />
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <TextArea
                  ...
                  onChange={(value) => setDraft({ ...draft, observedFacts: value })}
                  placeholder="Facts, messages or events you directly saw."
                  minRows={4}
                />
                <TextArea
                  ...
                  onChange={(value) => setDraft({ ...draft, uncertainties: value })}
                  placeholder="Separate suspicions or things you cannot verify yourself."
                  minRows={4}
                />
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <TextField
                  ...
                  onChange={(value) => setDraft({ ...draft, relatedCountries: value })}
                  placeholder="Oland, Vendia"
                  helper="Comma-separated. Leave blank if not relevant."
                />
                <TextField
                  label="Edition"
                  optional
                  value={draft.editionReference}
                  onChange={(value) => setDraft({ ...draft, editionReference: value })}
                  placeholder="SSC 20"
                  helper="Use whatever edition reference you know."
                />
              </div>
                ...
                <div className="flex gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold">
                      Evidence files are not enabled in this first secure slice
                    </p>
                    <p className="mt-1 text-xs leading-5 text-muted-foreground">
                      Describe what you have here. The two-way case thread already lets TSBC request
                      more information. File uploads need their own metadata stripping, access
                      controls and redaction path, so they are deliberately not being bolted on
                      carelessly.
                    </p>
                  </div>
                </div>
              ...
              <div className="flex flex-wrap justify-end gap-2">
                <button
                  type="button"
                  onClick={cancel}
                  className="min-h-11 rounded-xl border border-white/[0.08] bg-white/[0.025] px-4 text-sm font-semibold text-muted-foreground transition hover:text-foreground"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="min-h-11 rounded-xl bg-emerald-200 px-4 text-sm font-black text-emerald-950 transition hover:bg-emerald-100 disabled:cursor-not-allowed disabled:opacity-40"
                >
                  Review report <ArrowRight className="ml-1 inline size-4" />
                </button>
              </div>
            </div>
          ) : (
        ...
        <aside className="space-y-3 xl:sticky xl:top-24 xl:self-start">
          ...
          <div className="rounded-[1.4rem] border border-white/[0.08] bg-white/[0.025] p-4">
            ...
            <p className="mt-1.5 text-xs leading-5 text-muted-foreground">
              Submit information you genuinely believe may matter. A concern that cannot later be
              proven is not automatically a false report.
            </p>
            <Link
              to="/rules/$ruleId"
              params={{ ruleId: "16.2" }}
            ...
            >
              Read reporting rule <ArrowRight className="size-3.5" />
            </Link>
          </div>
        </aside>
      </div>
    ...
    <div className="rounded-[1.4rem] border border-emerald-300/14 bg-emerald-300/[0.045] p-4">
      ...
      <div className="mt-3 space-y-2 text-xs leading-5 text-muted-foreground">
        <p className="flex gap-2">
          ...
          The case record stores no reporter account ID.
        </p>
        <p className="flex gap-2">
          <Check className="mt-0.5 size-3.5 shrink-0 text-emerald-200" />A sessionless server
          connection submits the report.
        </p>
        <p className="flex gap-2">
          <Check className="mt-0.5 size-3.5 shrink-0 text-emerald-200" />
          Only a hashed recovery secret is stored.
        </p>
      </div>
      <p className="mt-3 text-[10px] leading-4 text-muted-foreground/80">
        Do not put your name, username or other identifying details into the report text if you want
        the content itself to remain anonymous.
      </p>
    </div>
  );
}
...
function ReportReview({
  ...
  return (
    <div className="mt-6 space-y-4">
      ...
      {draft.observedFacts ? (
        <ReviewRow label="Personally observed" value={draft.observedFacts} />
      ) : null}
      {draft.uncertainties ? (
        <ReviewRow label="Uncertain / suspected" value={draft.uncertainties} />
      ) : null}
      {draft.relatedCountries ? (
        <ReviewRow label="Countries" value={draft.relatedCountries} />
      ) : null}
      ...
      <div className="rounded-2xl border border-sky-300/14 bg-sky-300/[0.045] p-4 text-xs leading-5 text-muted-foreground">
        By submitting, you confirm only that you are providing the information in good faith. You
        are not being asked to prove guilt or guarantee that TSBC will find a violation.
      </div>
      ...
      <div className="flex flex-wrap justify-between gap-2 pt-2">
        <button
          ...
          className="min-h-11 rounded-xl border border-white/[0.08] bg-white/[0.025] px-4 text-sm font-semibold text-muted-foreground transition hover:text-foreground"
        >
          <ArrowLeft className="mr-1 inline size-4" /> Edit report
        </button>
        <button
          type="button"
          onClick={submit}
          disabled={submitting}
          className="min-h-11 rounded-xl bg-emerald-200 px-4 text-sm font-black text-emerald-950 transition hover:bg-emerald-100 disabled:opacity-50"
        >
    ...
    >
      ...
      <p
        className={cn(
          ...
          emphasis && "font-bold text-emerald-100",
        )}
      >
        {value}
      </p>
...
function AnonymousCaseAccess({
  ...
  defaultRecoveryKey: string;
  onCaseCode: (value: string) => void;
  onRecoveryKey: (value: string) => void;
  onOpen: (credential: AnonymousCaseCredential, snapshot?: AnonymousCaseSnapshot) => void;
}) {
  const getCase = useServerFn(getAnonymousIntegrityCase);
  const mutation = useMutation({
    mutationFn: () =>
      getCase({ data: { caseCode: defaultCaseCode, recoveryKey: defaultRecoveryKey } }),
    onSuccess: (result) => {
      ...
      const credential = {
        caseCode: defaultCaseCode.trim().toUpperCase(),
        recoveryKey: defaultRecoveryKey.trim(),
        savedAt: new Date().toISOString(),
      };
  ...
  return (
    <section className="rounded-[1.6rem] border border-sky-300/14 bg-sky-300/[0.035] p-5 sm:p-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="font-black">Open an anonymous case</h2>
          <p className="mt-0.5 text-xs text-muted-foreground">
            Your code and recovery key are required together.
          </p>
        </div>
      </div>
      <div className="mt-5 grid gap-3 md:grid-cols-[.8fr_1.2fr_auto]">
        <TextField
          label="Case code"
          value={defaultCaseCode}
          onChange={onCaseCode}
          placeholder="AR-1A2B3C4D5E"
        />
        <TextField
          label="Recovery key"
          value={defaultRecoveryKey}
          onChange={onRecoveryKey}
          placeholder="ABCD-EF12-3456-7890-ABCD-EF12"
        />
        <button
          type="button"
          disabled={mutation.isPending || !defaultCaseCode.trim() || !defaultRecoveryKey.trim()}
          onClick={() => mutation.mutate()}
          className="mt-[1.55rem] min-h-11 rounded-xl bg-sky-200 px-4 text-sm font-black text-slate-950 disabled:opacity-40"
        >
...
function AnonymousCaseView({
  credential,
  snapshot,
  setSnapshot,
  back,
}: {
  credential: AnonymousCaseCredential | null;
  snapshot: AnonymousCaseSnapshot | null;
  setSnapshot: (snapshot: AnonymousCaseSnapshot | null) => void;
  back: () => void;
}) {
  const getCase = useServerFn(getAnonymousIntegrityCase);
  ...
  const loadMutation = useMutation({
    mutationFn: () => {
      ...
      return getCase({
        data: { caseCode: credential.caseCode, recoveryKey: credential.recoveryKey },
      });
    },
    onSuccess: (result) => {
      if (!result.ok || !result.snapshot) {
  ...
  const replyMutation = useMutation({
    mutationFn: () => {
      if (!credential) throw new Error("Anonymous case credentials are missing.");
      return reply({
        data: { caseCode: credential.caseCode, recoveryKey: credential.recoveryKey, body: message },
      });
    },
    onSuccess: (result) => {
      if (!result.ok || !result.snapshot) {
  ...
  if (!credential) {
    return (
      <div className="mt-8 rounded-2xl border border-white/[0.08] bg-white/[0.02] p-6">
        ...
        <button type="button" onClick={back} className="mt-3 text-sm font-bold text-sky-200">
          Back to Integrity Centre
        </button>
      </div>
    );
  }
  ...
  if (!snapshot) {
    return (
      <div className="mt-8 rounded-[1.6rem] border border-white/[0.08] bg-white/[0.025] p-8 text-center">
        <RefreshCw
          className={cn("mx-auto size-6 text-sky-200", loadMutation.isPending && "animate-spin")}
        />
        ...
        {loadMutation.isError ? (
          <button
            ...
            className="mt-4 rounded-xl bg-sky-200 px-4 py-2 text-sm font-black text-slate-950"
          >
            Try again
          </button>
        ) : null}
      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_19rem]">
        <main className="min-w-0 space-y-4">
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  {category.label} · created {new Date(snapshot.case.created_at).toLocaleString()}
                </p>
              </div>
              <span className="rounded-full border border-sky-300/14 bg-sky-300/[0.06] px-3 py-1.5 text-xs font-bold text-sky-100">
                {status.label}
              </span>
            </div>
          ...
          <div className="rounded-[1.7rem] border border-white/[0.08] bg-[#081a35]/82 p-4 sm:p-6">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[.15em] text-sky-200/75">
                  Secure thread
                </p>
                <h3 className="mt-1 text-xl font-black">Reporter ↔ TSBC</h3>
              </div>
              <button
                type="button"
                onClick={() => loadMutation.mutate()}
                disabled={loadMutation.isPending}
                className="grid size-10 place-items-center rounded-xl border border-white/[0.08] bg-white/[0.025] text-muted-foreground transition hover:text-foreground"
              >
                <RefreshCw className={cn("size-4", loadMutation.isPending && "animate-spin")} />
              </button>
            </div>
            ...
            <div className="mt-5 space-y-3">
              {snapshot.messages.map((item) => (
                <div
                  key={item.id}
                  className={cn(
                    "max-w-[90%] rounded-2xl border p-4 sm:max-w-[78%]",
                    item.author_role === "reporter"
                      ? "ml-auto border-emerald-300/14 bg-emerald-300/[0.055]"
                      : "border-sky-300/14 bg-sky-300/[0.055]",
                  )}
                >
                  <div className="flex items-center justify-between gap-3 text-[10px] font-black uppercase tracking-[.12em] text-muted-foreground">
                    <span>{item.author_role === "reporter" ? "You · Anonymous" : "TSBC"}</span>
            ...
            <div className="mt-5 rounded-2xl border border-white/[0.08] bg-black/10 p-3">
              <textarea
                ...
                placeholder="Add information or answer TSBC anonymously…"
                rows={4}
                className="w-full resize-y border-0 !bg-transparent px-2 py-1 text-sm leading-6 shadow-none outline-none placeholder:text-muted-foreground/70 focus-visible:!shadow-none"
              />
              <div className="mt-2 flex items-center justify-between gap-3 border-t border-white/[0.06] pt-3">
                <p className="text-[10px] leading-4 text-muted-foreground">
                  Your message is stored as “Anonymous Reporter”.
                </p>
                <button
                  type="button"
                  onClick={() => replyMutation.mutate()}
                  disabled={replyMutation.isPending || message.trim().length < 2}
                  className="inline-flex min-h-10 shrink-0 items-center gap-2 rounded-xl bg-emerald-200 px-3 text-xs font-black text-emerald-950 disabled:opacity-40"
                >
        ...
        <aside className="space-y-3 xl:sticky xl:top-24 xl:self-start">
          <div className="rounded-[1.4rem] border border-emerald-300/14 bg-emerald-300/[0.045] p-4">
            ...
            <p className="mt-1 text-xs leading-5 text-muted-foreground">
              The key is the only credential that lets this anonymous mailbox be opened.
            </p>
            <button
              type="button"
              onClick={() => setShowKey((value) => !value)}
              className="mt-3 text-xs font-bold text-emerald-200"
            >
              {showKey ? "Hide key" : "Show key"}
            </button>
            {showKey ? (
              <code className="mt-2 block break-all rounded-xl border border-white/[0.07] bg-black/20 p-3 text-[11px] text-slate-200">
            ...
            <button
              ...
              onClick={async () => {
                ...
                toast.success("Case credentials copied");
              }}
              className="mt-3 inline-flex items-center gap-2 text-xs font-bold text-sky-200"
            >
          ...
          <div className="rounded-[1.4rem] border border-white/[0.08] bg-white/[0.025] p-4">
            ...
            <div className="mt-3 space-y-3">
              {snapshot.events
                ...
                .reverse()
                .map((event) => (
                  <div key={event.id} className="border-l border-white/[0.09] pl-3">
                    <p className="text-xs font-semibold">{event.detail ?? event.event_type}</p>
          ...
          <Link
            to="/rules/$ruleId"
            params={{ ruleId: "16.6" }}
            className="block rounded-[1.4rem] border border-violet-300/14 bg-violet-300/[0.04] p-4 transition hover:bg-violet-300/[0.07]"
          >
            <ShieldCheck className="size-5 text-violet-200" />
            <p className="mt-3 text-sm font-bold">Case fairness</p>
            <p className="mt-1 text-xs leading-5 text-muted-foreground">
              A report and an investigation are not findings of misconduct.
            </p>
          </Link>
        </aside>
      </div>
...
function TextField({
  ...
  return (
    <label className="block">
      ...
      />
      {helper ? (
        <span className="mt-1.5 block text-[10px] leading-4 text-muted-foreground">{helper}</span>
      ) : null}
    </label>
  );
}
...
function SectionHeading({
  ...
  return (
    <div>
      <p className="text-[10px] font-black uppercase tracking-[.18em] text-emerald-200/75">
        {eyebrow}
      </p>
