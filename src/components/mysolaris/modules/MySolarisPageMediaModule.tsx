...
export function MySolarisPageMediaModule() {
  ...
  if (isLoading) {
    return (
      <AppShell>
        <p className="text-sm text-muted-foreground">Loading page & media…</p>
      </AppShell>
    );
  }
  if (!country) {
...
function CountryPageBuilder({
  ...
  return (
    <AppShell>
      <PageHeader
        eyebrow="My country · Page & media"
        title={`${country.name} page & media`}
        description="Build the public country profile and Wiki from the same blocks, then manage reusable images and appearance without leaving MySolaris."
        actions={
