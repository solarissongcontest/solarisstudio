    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-3 sm:grid-cols-2">
        <Field label="Opening title">
          <TextInput value={config.openingTitle} placeholder="Solaris Song Contest" onChange={(e) => set({ openingTitle: e.target.value })} />
        </Field>
        <Field label="Opening subtitle">
          <TextInput value={config.openingSubtitle} placeholder="Grand Final" onChange={(e) => set({ openingSubtitle: e.target.value })} />
        </Field>
      ...
      <section className="space-y-3">
        ...
        <Field label="Point animation delay">
          <Slider min={120} max={2000} step={20} value={config.pointDelay} onChange={(v) => set({ pointDelay: v })} suffix="ms" />
        </Field>
        <Field label="Pause between voting countries">
          <Slider min={200} max={5000} step={100} value={config.spokespersonDelay} onChange={(v) => set({ spokespersonDelay: v })} suffix="ms" />
        </Field>
        <Field label="Top-award hold">
          <Slider min={0} max={5000} step={100} value={config.topPointHold} onChange={(v) => set({ topPointHold: v })} suffix="ms" />
        </Field>
        <Field label="Televote reveal delay">
          <Slider min={200} max={6000} step={100} value={config.televoteHold} onChange={(v) => set({ televoteHold: v })} suffix="ms" />
        </Field>
      </section>
