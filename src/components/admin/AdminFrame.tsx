...
export function AdminFrame({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] lg:grid lg:grid-cols-[238px_minmax(0,1fr)]">
      ...
      <main className="min-w-0 p-3 pb-24 sm:p-5 lg:p-6 lg:pb-6">{children}</main>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-[120] lg:hidden">
          <button type="button" className="absolute inset-0 bg-black/65 backdrop-blur-sm" aria-label="Close organizer navigation" onClick={() => setMobileOpen(false)} />
          <aside className="absolute inset-y-0 left-0 w-[min(86vw,310px)] overflow-y-auto border-r border-border bg-background">
            ...
            <div onClick={() => setMobileOpen(false)}><AdminNav /></div>
          </aside>
        </div>
      )}
          ...
          {mobileItems.map((item) => {
            ...
            return (
              <Link
                key={item.label}
                to={item.href as any}
                className="flex min-h-12 flex-col items-center justify-center gap-1 rounded-lg px-1 text-[10px] font-semibold text-muted-foreground hover:bg-surface hover:text-foreground"
              >
