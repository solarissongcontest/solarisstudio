...
export function AdminFrame({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] xl:grid xl:grid-cols-[248px_minmax(0,1fr)]">
      <aside className="admin-sidebar hidden border-r border-white/[0.07] xl:block">
        <div className="sticky top-16 max-h-[calc(100vh-4rem)] overflow-y-auto scroll-slim">
          <AdminNav />
        </div>
      </aside>

      <main className="admin-page min-w-0 p-3 pb-24 sm:p-5 md:pb-6 lg:px-7 lg:py-6 xl:px-8 2xl:px-10">
        {children}
      </main>

      <nav
        className="admin-mobile-nav fixed inset-x-0 bottom-0 z-50 border-t border-white/[0.08] px-2 pt-2 md:hidden"
